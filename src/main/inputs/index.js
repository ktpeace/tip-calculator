...
const Bill = ({billTotal, setBillTotal}) => {
  const billChangeHandler = (event) => {
    // TODO FIX VALIDATING INPUTS HERE
...
const Tip = ({tipAmount, setTipAmount, billTotal}) => {
  ...
  // update tipAmount, set customTip back to null
      ...
      <div className="tip-selection-div">
        {
          tipList.map(num => {
            ...
            let activeAmount = (Number(billTotal) !== 0 && billTotal*(num / 100) === tipAmount);
...
const Inputs = ({billTotal, setBillTotal, tipAmount, setTipAmount, numPeople, setNumPeople}) => {
  return (
    <div>
      <Bill billTotal={billTotal} setBillTotal={setBillTotal} />
      <Tip tipAmount={tipAmount} setTipAmount={setTipAmount} billTotal={billTotal}/>
